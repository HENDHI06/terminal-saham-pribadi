...
def draw_mobile_cards(df):
    for _, row in df.iterrows():
        ...
        sig_color = "#ccff00" if "BSJP" in row['REKOMENDASI'] else "#00ffff"
        st.markdown(f"""
        <div style="background: rgba(255, 255, 255, 0.05); border: 1px solid rgba(204, 255, 0, 0.2); 
            ...
            <div style="display: flex; justify-content: space-between; align-items: center;">
                ...
                <span style="color: {sig_color}; font-size: 0.7rem; border: 1px solid {sig_color}; padding: 2px 5px; border-radius:4px;">{row['REKOMENDASI']}</span>
            </div>
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 8px; margin-top: 10px; font-size: 0.8rem; color: #bbb;">
                ...
                try:
                    ...
                    if cond:
                        results.append({
                            ...
                            "REKOMENDASI": sig, "VAL(M)": round(val/1_000_000, 1), 
                            "ENTRY": int(c_now), "TP": int(c_now*1.03) if sig=="🚀 BSJP" else int(c_now*1.15),
                            "CL": int(c_now*0.97) if sig=="🚀 BSJP" else int(ma50), "FULL": t
                        })
                except: continue
        except: continue
...
st.sidebar.markdown(f"<div style='padding:10px; border:1px solid #ccff0033; border-radius:10px;'><b>{user_now.upper()}</b><br><small>{role.upper()}</small></div>", unsafe_allow_html=True)
menu_list = ["STRATEGY SCANNER", "FUNDAMENTAL ANALYZER", "MONEY MANAGEMENT", "SECURITY SETTINGS"]
...
if menu == "STRATEGY SCANNER":
    ...
    try:
        ...
        diff_ihsg = curr_ihsg - ihsg['Close'].iloc[-2]
        st.markdown(f"IHSG: **{curr_ihsg:,.2f}** ({diff_ihsg:+.2f})")
    except: pass
    ...
    if 'results' in st.session_state and not st.session_state.results.empty:
        ...
        c1, c2 = st.columns(2)
        with c1:
            st.success("🔥 BEST FOR BSJP")
            for t in df[df['REKOMENDASI']=="🚀 BSJP"]['TICKER'].head(3): st.write(f"**{t}**")
        with c2:
            st.info("🏆 BEST FOR HOLD")
            for t in df[df['REKOMENDASI']=="💎 HOLD"]['TICKER'].head(3): st.write(f"**{t}**")

        st.markdown("---")
        t_desk, t_mob = st.tabs(["🖥️ DESKTOP", "📱 MOBILE"])
        with t_desk: st.dataframe(df.drop(columns=['FULL']), use_container_width=True, hide_index=True)
        with t_mob: draw_mobile_cards(df)
