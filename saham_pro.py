...
# --- KONEKSI GSHEETS (TAMBAHAN UNTUK PERMANEN) ---
...
# Database lokal tetap untuk USER & LOGIN (Cepat & Privat)
def init_db():
...
def sell_position(u, ticker, buy_p, sell_p, lots, row_index):
    # Simpan ke History GSheets
    ...
    save_gsheets_data(df_h, "history")
    
    # Hapus dari Portfolio GSheets
    df_p = load_gsheets_data("portfolio")
    df_p = df_p.drop(row_index)
    save_gsheets_data(df_p, "portfolio")
...
if not st.session_state["auth"]["logged_in"]:
    ...
    with col2:
        st.markdown("<div style='text-align:center; padding:50px 0;'><h1>IDX</h1><p style='letter-spacing:5px;'>CYBER TERMINAL</p></div>", unsafe_allow_html=True)
        ...
        try:
            ...
            for t in batch:
                try:
                    ...
                    val = c_now * float(last['Volume'])
                    if (mode == "Ketat" and chg > 3 and val > 1e9) or (mode == "Agresif" and chg > 1.5):
                        results.append({
                            "TICKER": t.replace(".JK",""), "LAST": int(c_now), "CHG%": round(chg, 2), 
                            "VOL_S": "⚡ SPIKE", "ENTRY": f"{int(c_now)}", "TP": int(c_now*1.03), 
                            "CL": int(c_now*0.97), "VAL(M)": round(val/1e6, 1), "FULL": t
                        })
                except: continue
        except: continue
...
st.sidebar.markdown(f"<div style='padding:15px; border:1px solid #ccff0033; border-radius:10px; background:rgba(204,255,0,0.05);'><h3>{user_now.upper()}</h3><p style='font-size:10px;'>{loc_l}</p></div>", unsafe_allow_html=True)
...
# --- 6. CONTENT (MONEY MANAGEMENT - PERMANEN) ---
    ...
    with t1:
        with st.expander("➕ ADD NEW POSITION"):
            with st.form("add_p"):
                ...
                t_in = c1.text_input("Ticker"); p_in = c2.number_input("Price"); l_in = c3.number_input("Lots", min_value=1)
        ...
        if not df_p.empty:
            ...
            for idx, row in df_p.iterrows():
                if st.button(f"🚀 SELL {row['ticker']}", key=f"btn_{idx}"):
                    sell_position(user_now, row['ticker'], row['buy_price'], row['buy_price'], row['lots'], idx)
                    st.rerun()
        else: st.info("Portfolio kosong (Data GSheets).")
...
# --- 7. USER & SECURITY ---
    ...
    conn = sqlite3.connect('users.db')
    st.dataframe(pd.read_sql_query("SELECT username, role, last_login FROM users", conn), use_container_width=True)
